...
def qwen_optimized_test_class_alg(target, data_test, data_train, data_train_means, fourier_coef, tmp_test_coef, rewrite=True):
...
def batch_get_labels(target, data_train, data_train_means, fourier_coef, test_images, tmp_test_coef, is_labeled = True):
    list_of_labels = np.sort(data_train[target].unique())
    dim = (data_train.shape[1] - 1) // 2
    num_classes = len(list_of_labels)
